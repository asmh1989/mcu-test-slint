    ...
    pub async fn send_and_receive(
        ...
        if let Some(port) = port_guard.as_mut() {
            ...
            match port.write_all(command).await {
                Ok(_) => {}
                Err(e) => {
                    log::error!("发送SCPI命令 ({}): 失败: {}", self.port_path, e);
            ...
            .await
            {
                Ok(Ok(data)) => {
                    ...
                    log::debug!("接收SCPI响应 ({}): {}", self.port_path, response_str);
                    Ok(response_str)
                }
                Ok(Err(e)) => {
                    log::error!("读取SCPI响应失败 ({}): {}", self.port_path, e);
                    Err(e)
                }
                Err(_) => {
                    log::warn!("读取SCPI响应超时 ({})", self.port_path);
